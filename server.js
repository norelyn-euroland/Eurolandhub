...
app.post('/api/generate-document-summary', (req, res) => {
  ...
  // JSON body path: { text: "..." } or { csvText: "..." }
  if (contentType.includes('application/json')) {
    const { text, csvText } = req.body || {};
    const documentText = (text || csvText || '').trim();
    if (!documentText) {
      return res.status(400).json({ success: false, error: 'No text provided.' });
    }
    return summarizeText(documentText, res);
  }
  ...
  busboy.on('finish', async () => {
    ...
    try {
      ...
      console.error('[DocSummary] Error:', e);
  ...
  busboy.on('error', (err) => {
    console.error('[DocSummary] Busboy error:', err);
    res.status(500).json({ success: false, error: 'Failed to parse file upload', detail: err.message });
  });

  req.pipe(busboy);
});

// Shared helper: truncate text, call Groq, return summary
async function summarizeText(documentText, res) {
  const groqKey = process.env.GROQ_API_KEY;
  if (!groqKey) {
